...
vec4 filterBlur3x3(sampler2D ctex, vec2 coord) {
    ...
    for (float x = -0.5; x <= 0.6; x+=1) {
        for (float y = -0.5; y <= 0.6; y+=1) {
            col += texture2D(ctex, vec2(x,y) * screenSizeInverse + coord);
        }
    }
    return col * 0.25;
}
...
vec4 filterBlurDepth(sampler2D ctex, sampler2D dtex, vec2 coord) {
    vec4  col   = vec4(0);
    float depth = linearizeDepthf(texture2D(dtex, coord).x, nearInverse);
    float accu  = 0.0;
    for (int x = 0; x < 5; x++) {
        for (int y = 0; y < 5; y++) {
            vec2  offs   = (vec2(x,y) - 2) * screenSizeInverse;
            vec4  sCol   = texture2D(ctex, coord + offs);
            float sDepth = linearizeDepthf(texture2D(dtex, coord + offs).x, nearInverse);
