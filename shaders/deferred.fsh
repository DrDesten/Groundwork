...
float SimpleSSAO(vec2 coord, float radius, float dither) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        vec2  offs = spiralOffset(i + dither + 0.5, 16) * radius;
...
/* DRAWBUFFERS:3 */
void main() {
    vec3 lighting = texture2D(colortex3, coord).rgb;

    lighting *= vec3(SimpleSSAO(coord, 1, Bayer8(coord * screenSize) * 0.1));// * 0.5 + 0.5;
